    ...
    <Section id="features" background="gray" ariaLabelledby="features-title">
      <Container>
        <div className="text-center mb-6">
          ...
          <p className="text-lg md:text-xl lg:text-lg tracking-tight leading-relaxed mb-2">
        ...
        <Grid cols={3} gap="large" className="mb-12">
          {FEATURES.map((feature, index) => (
            <Card key={index} variant="elevated" padding="large" hover className="text-center">
              ...
              <p className="text-base text-primary-muted mb-3 text-sm md:text-base tracking-tight">
                {feature.hashtag}
              </p>
              <ul className="space-y-1 md:space-y-2 text-left">
                {feature.points.map((point, pointIndex) => (
                  <li key={pointIndex} className="text-gray-700 text-sm md:text-base tracking-tight">
                    • {point}
                  </li>
                ))}
              </ul>
        ...
        <div className="text-center mt-2">
          <p className="text-lg tracking-tight">아이의 변화를 직접 경험해보세요.</p>
        </div>

        <div className="text-center mt-8">
          <p className="text-sm tracking-tight text-neutral-900 mb-2">
