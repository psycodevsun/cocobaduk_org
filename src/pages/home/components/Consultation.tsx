...
const Consultation = memo(function consultation() {
  return (
    <Section id="consultation" background="white" ariaLabelledby="consultation-title" className='bg-gradient-to-tr from-[#418fc9] to-[#0a4272] max-md:pb-10 pb-12'>
      <Container size="wide">
        ...
        <div className='bg-white w-[40%] text-center p-5 rounded-2xl mt-12 m-auto max-md:w-full'>
          <h3 className='font-bold text-[#2864a0] text-xl'>지금 바로 전화주세요</h3>
          <a href='tel:010-6325-8985' className='block bg-[#f23991] w-[70%] text-white m-auto text-lg rounded-full font-black tracking-tight mt-2 hover:bg-[#c9377c]'>010-6325-8985</a>
          <p className='text-gray-600 mt-2 text-xs tracking-tighter mb-1'>핸드폰에서 클릭하시면 전화 연결이 가능합니다</p>
        </div>

        <div className='bg-white w-[40%] text-center p-5 rounded-2xl mt-12 m-auto max-md:w-full max-md:mt-6'>
          <h3 className='font-bold text-[#2864a0] text-xl'>간단한 상담 신청만 남겨주세요</h3>
          <a href='https://forms.gle/5hcuKZivWkemG3476' target='_blank' className='block bg-[#f23991] w-[70%] text-white m-auto text-lg rounded-full font-bold tracking-tight mt-2 hover:bg-[#c9377c]'>상담신청 남기기 클릭 !</a>
          <p className='text-gray-600 mt-2 text-xs tracking-tighter mb-1'>확인후 순차적으로 연락드립니다</p>
        </div>

        <p className='text-white mt-8 font-bold text-base text-center'>지금, 부담없이 시작해보세요</p>
      </Container>
    </Section>
  );
});
